...
function onDrop(event, form_id, upload_form_name, csrf_token, upload_end_msg, upload_error_msg, form_add_opt, method, timeout, redirect_url, result_id) {
...
//	if (files.length == 1) {
        for (var i=0; i<files.length; i++) {
            FileUpload(form_id, upload_form_name, files[i], csrf_token, upload_end_msg, upload_error_msg, form_add_opt, method, timeout, redirect_url, i, result_id);
	}
}
 
//ファイルアップロード
function FileUpload(form_id, upload_form_name, upload_file, csrf_token, upload_end_msg, upload_error_msg, form_add_opt, method, timeout, redirect_url, number, result_id) {
	...
	//フォームデータの作成
	var formData = new FormData(document.forms.form_id);
	...
	if( form_add_opt instanceof Array ){
            for(var i= 0;i<form_add_opt.length;i++){
                formData.append(form_add_opt[i], $('#'+form_id+' input[name=' + form_add_opt[i] + ']').val());	
            }
	}
 ...
 //   Ajax(form_id, method, csrf_token, formData, upload_end_msg, upload_error_msg, timeout);
	...
	var p_id = 'progress' + number;
	var f_id = 'upload_status' + number;
        var stat_id = 'status' + number;
        var result_status = 'result';
        if( result_id != undefined ){
            result_status = result_id;
        }
	$('#'+result_status).append('<div style="font-color:black;background:white;height:25px;font-family:Meiryo;">アップロード状況：<span id="'+ f_id + '"></span><progress id="'+ p_id +'" value="0" max="100">0%</progress><span id="'+ stat_id +'"></span></div>');
	...
	$.ajax($('#' + form_id).prop('action'), {
		...
		error: function(error) {
                    list_error= JSON.parse(error.responseText);
