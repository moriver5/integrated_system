				...
				<div class="panel-body">
					...
					<table border="1" align="center" width="99%">
						<tr>
							...
							<td class="admin_table" style="width:260px;">
								<b>MEMO</b>
							</td>
							<td class="admin_table" style="width:30px;">
								<b>参照</b>
								...
								<tr>
									...
									<td style="padding:2px;text-align:center;">
										@if( !empty($lines->open_flg) && !empty($lines->url_open_flg) )
											<a href="{{ $base_url }}/{{ config('const.redirect_landing_url_path') }}/{{ $lines->id}}/{{$lines->name}}" class="lp_preview"><b>{{ $base_url }}/{{ config('const.redirect_landing_url_path') }}/{{ $lines->id}}/?afl=****</b></a>
										@else
											<font color="darkgray">{{ $base_url }}/{{ config('const.redirect_landing_url_path') }}/{{ $lines->id}}/?afl=****</font>
										@endif
									</td>
									<td style="padding:2px;text-align:center;">
										{{ $lines->memo }}
									...
									<td style="padding:2px;text-align:center;">
										<!-- 公開するときのみ表示 -->
										@if( !empty($lines->open_flg) )
										<a href="/admin/member/lp/create/content/{{ $lines->id }}/0/index" target="_blank"><b>pc</b></a> / <a href="/admin/member/lp/create/content/{{ $lines->id }}/2/index" target="_blank"><b>sp</b></a> / <a href="/admin/member/lp/create/img/{{ $lines->id }}" target="_blank"><b>画像</b></a>
										@endif
									</td>
								</tr>
							@endforeach
...
$(document).ready(function(){
	...
	$('#create').on('click', function(){
		search_win = window.open('/admin/member/lp/create', 'create', 'width=500, height=415');
		return false;
	});
