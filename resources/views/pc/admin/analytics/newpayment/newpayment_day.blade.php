        ...
        <div class="col-md-12 col-md-offset">
            <div class="panel panel-default">
                ...
                <div class="panel-heading" style="font:normal 12px/120% 'メイリオ',sans-serif;text-align:center;">
					<b><a href="/admin/member/analytics/newpayment/{{ $year }}/{{$prev_month}}/{{$prev_day}}">PREV</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/admin/member/analytics/newpayment/{{ $year }}/{{ $month }}">{{ $year }}年{{ $month }}月</a>{{ $day }}日&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/admin/member/analytics/newpayment/{{ $year }}/{{$next_month}}/{{$next_day}}">NEXT</a></b>
				</div>
						...
						<table border="1" align="center" style="width:100%;">
							...
							@foreach($db_data as $index => $lines)
								<tr style="font:12px/120% 'メイリオ',sans-serif;">
									...
									<td style="padding:3px;text-align:center;" class="pay_date" id="pay_date{{ $index }}">
										{{ $lines->pay_date }}
									</td>
									...
									<td style="padding:3px;text-align:center;" class="apr_count">
										{{ preg_replace("/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/","$1-$2-$3 $4:$5:$6",$lines->user_regist_date) }}
									</td>
									<td style="padding:3px;text-align:center;" class="period_day" id="period_day{{ $index }}">
...
$(document).ready(function(){
	...
	$.when(
		...
		$('.period_day').each(function(){
			var pay_date_id = this.id.replace(/period_day(\d+)/,"$1");
			var date = $(this).text().replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/,"$1-$2-$3");
			var n = new Date(date);
			var m = new Date($("#pay_date"+pay_date_id).text());
