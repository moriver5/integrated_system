    ...
    <div class="col">
		<div class="panel panel-default" style="width:1400px;">
			...
			<div class="panel-heading" style="font:normal 12px/120% 'メイリオ',sans-serif;text-align:center;">
				<b><a href="/admin/member/analytics/payment/structure/{{$prev_year}}/{{ $prev_month }}">PREV</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/admin/member/analytics/payment/structure/{{$year}}">{{ $year }}年</a>{{ $month }}月&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/admin/member/analytics/payment/structure/{{$next_year}}/{{ $next_month }}">NEXT</a></b>
			</div>
						...
						@foreach($db_data as $day => $lines)
							<tr style="font:12px/120% 'メイリオ',sans-serif;">
								<td style="padding:3px;text-align:center;" class="pay">
									<a href="/admin/member/analytics/payment/structure/{{ $year }}/{{ $month }}/{{ $day }}">{{ $day }}日</a>
								</td>
...
$(document).ready(function(){
	...
	$.when(
		...
		var user_unit_price = Math.round(amount_count / user_count);
		$('#total_user_unit_price').text(user_unit_price.toLocaleString());

		var order_unit_price = Math.round(amount_count / order_count);
