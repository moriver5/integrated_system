							...
							<div>
								<div class="form-group" style="align:center;">
									<table border="1" width="97%">
										...
										<tr>
											<td class="admin_search" style="width:60px;">表示順</td>
											<td style="padding:5px;width:50px;" colspan="3">
												<select name="sort" class="form-control">
												@foreach($ad_sort_list as $index => $line)
													@if( !empty($session['media_sort']) && $index == $session['media_sort'] )
														<option value="{{ $index }}" selected>{{ $line[1] }}</option>
													@else
														<option value="{{ $index }}">{{ $line[1] }}</option>													
													@endif
												@endforeach
												</select>
											</td>
