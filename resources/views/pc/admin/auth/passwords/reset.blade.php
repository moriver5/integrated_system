                ...
                <div class="panel-body">
                    ...
                    <form id="formPasswordReset" class="form-horizontal" method="POST" action="/admin/forget">
                        ...
                        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="email" type="email" class="form-control" name="email" value="{{ $email or old('email') }}" maxlength="{{ config('const.email_length') }}" placeholder="登録したメールアドレスを入力してください" required autofocus>
...
$(document).ready(function(){
	...
	submitAlert('formPasswordReset', 'post', '{{ __('messages.dialog_alert_title') }}', '{{ __('messages.dialog_alert_msg') }}', '{{ __('messages.password_setting') }}', '{{ __('messages.cancel_msg') }}', {{ config('const.admin_default_ajax_timeout') }}, true);
});
</script>
