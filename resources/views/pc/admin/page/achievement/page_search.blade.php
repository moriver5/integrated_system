									...
									<table border="1" width="97%">
										<tr>
											...
											<td style="width:100px;padding:5px;width:40px;">
												...
												@foreach($page_race_search_item as $lines)
													@if( !empty($session['page_race_search_item']) && $lines[0] == $session['page_race_search_item'] )
											...
											<td style="width:100px;padding:5px;">
												<!-- 検索項目の値 -->
												@if( !empty($session['page_race_search_item_value']) )
													<input id="search_item_value" type="text" class="form-control" name="search_item_value" value="{{ $session['page_race_search_item_value'] }}" autofocus>
												@else
										...
										<tr>
											...
											<td colspan="2" style="padding:5px;">
												@if( !empty($session['start_race_date']) )
													&nbsp;&nbsp;<input id="start_race_date" type="text" name="start_race_date" value="{{$session['start_race_date']}}" placeholder="開始日">
												@else
													&nbsp;&nbsp;<input id="start_race_date" type="text" name="start_race_date" placeholder="開始日">
												@endif
												@if( !empty($session['end_race_date']) )
													&nbsp;&nbsp;～&nbsp;&nbsp;<input id="end_race_date" type="text" name="end_race_date" value="{{$session['end_race_date']}}" placeholder="終了日">
												@else
													&nbsp;&nbsp;～&nbsp;&nbsp;<input id="end_race_date" type="text" name="end_race_date" placeholder="終了日">
												@endif
											</td>
										</tr>
										<tr>
											...
											<td colspan="2" style="padding:5px;">
												@foreach($page_race_disp_type as $index => $line)
													@if( isset($session['page_race_disp_type']) && $index == $session['page_race_disp_type'] )
										...
										<tr>
											...
											<td colspan="2" style="padding:5px;">
												@foreach($sort_list as $index => $line)
													@if( isset($session['page_race_sort']) && $index == $session['page_race_sort'] )
...
$(document).ready(function(){
	...
	//登録日時-開始日
	$('#start_race_date').datetimepicker({format:'Y-m-d'});

	//登録日時-終了日
	$('#end_race_date').datetimepicker({format:'Y-m-d'});
	...
	$('#search_setting').on('click', function(){	
		...
		//開催日-開始日
        fm.start_race_date.value = $('[name="start_race_date"]').val();

		//開催日-終了日
        fm.end_race_date.value = $('[name="end_race_date"]').val();

		//開催地
        fm.venue.value = $('[name="venue"]').val();
