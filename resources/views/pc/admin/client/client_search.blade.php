							...
							<div>
								<div class="form-group" style="align:center;">
									...
									@foreach($db_group_data as $index => $lines)
										...
										@if( isset($session['group_id']) && preg_match("/^(".preg_replace("/,/", "|",$session['group_id']).")$/",$lines[0]) > 0 )
											&nbsp;<input type="checkbox" name="group_id" value="{{ $lines[0] }}" checked>{{ $lines[1] }}
										@else
											&nbsp;<input type="checkbox" name="group_id" value="{{ $lines[0] }}">{{ $lines[1] }}									
										@endif
									@endforeach
											</td>
										</tr>
										<tr>
											...
											<td colspan="3" style="padding:5px;">
												@foreach($regist_status as $index => $lines)
													@if( isset($session['reg_status']) && preg_match("/^(".preg_replace("/,/","|",$session['reg_status']).")$/",$index) > 0 )
														&nbsp;&nbsp;<input type="checkbox" name="reg_status" value="{{ $index }}" checked>{{ $lines[1] }}
													@else
														&nbsp;&nbsp;<input type="checkbox" name="reg_status" value="{{ $index }}">{{ $lines[1] }}													
													@endif
												@endforeach
											</td>
										</tr>
										...
										<tr>
											<td style="text-align:center;background:wheat;font-weight:bold;">入金日時</td>
											<td colspan="3" style="padding:5px;">
								...
								<button type="button" class="btn btn-primary" id="setting_clear">条件リセット</button>
								<button type="submit" class="btn btn-primary" id="data_export">顧客データをエクスポート</button>
							</div>
						</center>
					</form>
                </div>
...
$(document).ready(function(){
	$('#setting_clear').on('click', function(){
		$('[name=search_item]').val("");
		$('[name=start_regdate]').val("");
		$('[name=end_regdate]').val("");
		$('[name=start_provdate]').val("");
		$('[name=end_provdate]').val("");
		$('[name=start_lastdate]').val("");
		$('[name=end_lastdate]').val("");
		$('[name=start_paydate]').val("");
		$('[name=end_paydate]').val("");
		$('[name="search_item"]').val("");
		$('[name="group_id"]:checked').map(function(){
            $(this).prop("checked", false);
        }).get();
		$('[name="reg_status"]:checked').map(function(){
            $(this).prop("checked", false);
        }).get();
		$('[name="dm_status"]').val("");
		$('[name="start_paynum"]').val("");
		$('[name="end_paynum"]').val("");
		$('[name="start_payamount"]').val("");
		$('[name="end_payamount"]').val("");
		$('[name="start_actnum"]').val("");
		$('[name="end_actnum"]').val("");
		$('[name="start_pt"]').val("");
		$('[name="end_pt"]').val("");
		$('[name="sort"]').val("");
	...
	$("#start_provdate").dblclick(function () {
	  $('[name=start_provdate]').val(dateFormat.format(new Date(), 'yyyy/MM/dd hh:mm'));
	});

	$("#end_provdate").dblclick(function () {
	  $('[name=end_provdate]').val(dateFormat.format(new Date(), 'yyyy/MM/dd hh:mm'));
	});

	$("#start_lastdate").dblclick(function () {
	  $('[name=start_lastdate]').val(dateFormat.format(new Date(), 'yyyy/MM/dd hh:mm'));
	});

	$("#end_lastdate").dblclick(function () {
	  $('[name=end_lastdate]').val(dateFormat.format(new Date(), 'yyyy/MM/dd hh:mm'));
	});
	...
	$('#data_export').on('click', function(){	
		//親ウィンドウのフォームオブジェクトを取得
		var fm = window.document.formSearchExport;
