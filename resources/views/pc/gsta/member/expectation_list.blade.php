...
<li>
@if( $lines['is_active'] === true )
<a href="{{ config('const.member_expectation_toll_path') }}/view/2/{{ $lines['forecast_id']}}"><p class="cpname">{{ $lines['title'] }}@if( !empty($lines['sub_title']) ) ({{ $lines['sub_title'] }})@endif</p><p class="btn">購入した情報を見る</p></a>
@else
<a tabindex="-1"><p class="cpname">{{ $lines['title'] }}@if( !empty($lines['sub_title']) ) ({{ $lines['sub_title'] }})@endif</p><p class="btn">情報公開の期間外です</p></a>
@endif
</li>
@endforeach
