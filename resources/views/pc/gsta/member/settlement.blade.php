...
@if( !empty($db_data) )
	...
	@foreach($db_data as $lines)
		<!-- 完売以外でユーザーと同じグループの商品を表示 -->
		@if( $lines['is_sold_out'] === false && ( empty($lines['groups']) || in_array($group_id, explode(",", $lines['groups'])) ) )
			<li>
			<input type="checkbox" id="id{{ $lines['id'] }}" name="product_id[]" value="{{ $lines['id'] }}" class="btn" {{ $checked }}>
			<label for="id{{ $lines['id'] }}">
			<span class="text">{{ $lines['title'] }}
			@if( !empty($lines['sub_title']) )
				 ({{ $lines['sub_title'] }})
			@endif
			</span>
