...
{
	/*
	 * ユーザーのログインIDが既に使用されているのかチェック
	 */
	public function validateCheckExistLoginId($attribute, $login_id, $rule)
	{
		$db_data = DB::connection(Session::get('operation_select_db'))
					->select("select * from users where login_id = {$login_id} and login_id != '' and id != {$rule[0]}");
	 ...
	 * ユーザーログインIDのチェック
	 */
	public function validateCheckUserLoginId($attribute, $login_id, $rule)
	{
		if( !empty($login_id) && preg_match("/^[0-9]{".config('const.login_id_length')."}$/", $login_id) == 0 ){
			return false;
		}
	...
	/*
	 * 空白以外で半角英数字と_-.のみ含まれているかどうかチェック
	 */
	public function validateCheckAlphaDash($attribute, $string, $rule)
	{
		if( !empty($string) && preg_match("/^[0-9a-z\-_\.]+$/", $string) == 0 ){
			return false;
		}
	...
	{
		$db_data = DB::connection(Session::get('operation_select_db'))->table('groups')->pluck('id');
	...
	public function validateExistCampaignIdCheck($attribute, $campaigns, $rule)
	{
		...
		$db_data = DB::connection(Session::get('operation_select_db'))->table('top_contents')->pluck('id');
	...
	{
		//サロゲートペアが含まれていればエラー
		if ( $str != '' && preg_match('/[\xd8-\xdb][\x00-\xff][\xdc-\xdf][\x00-\xff]/', mb_convert_encoding($str, 'UTF-16', 'UTF-8')) > 0 ) {
			return false;
		}
