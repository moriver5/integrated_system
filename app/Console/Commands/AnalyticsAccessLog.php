	...
	public function handle()
	{
		...
		if( count($listSiteDb) > 0 ){
			foreach($listSiteDb as $lines){
				...
				//時間帯別入金あり
				$listPay = DB::connection($db_name)->select(
				...
				//時間帯別入金なし
				$listNoPay = DB::connection($db_name)->select(
					"select date_format(created_at, '%Y/%m') as pay_date,"
					. "count(*) as count "
					. "from access_logs "
				...
				DB::connection($db_name)->transaction(function() use($db_name, $year, $month, $no_pay, $pay){
					DB::connection($db_name)->insert("insert ignore into year_result_access_logs("
						. "access_date, "
						. "no_pay, "
						. "pay, "
						. "total, "
				...
				//集計結果をyear_result_access_logsテーブルに登録
				DB::connection($db_name)->transaction(function() use($db_name, $year, $month, $day, $no_pay, $pay){
					DB::connection($db_name)->insert("insert ignore into month_result_access_logs("
						. "access_date, "
						. "no_pay, "
						. "pay, "
						. "total, "
				...
				//集計結果をyear_result_access_logsテーブルに登録
				DB::connection($db_name)->transaction(function() use($db_name, $year, $month, $day, $listData){
					foreach($listData as $hour => $line){
						...
						//集計結果をyear_result_access_logsテーブルに登録
						DB::connection($db_name)->insert("insert ignore into day_result_access_logs("
							. "access_date, "
							. "no_pay, "
							. "pay, "
							. "total, "
				...
				//24時間以上前のデータをすべて削除
				DB::connection($db_name)->transaction(function() use($db_name, $year, $month, $day, $hour){
					//24時間前の時刻を取得
					$listPrevDate = getdate(strtotime("{$year}-{$month}-{$day} {$hour}:0:0 -23 hour"));

					//使用しない24時間以上前のデータをすべて削除
					DB::connection($db_name)->delete("delete from access_logs where created_at < '{$listPrevDate['year']}-{$listPrevDate['mon']}-{$listPrevDate['mday']} {$listPrevDate['hours']}:0:0'");
				});
			}
		}
	}
