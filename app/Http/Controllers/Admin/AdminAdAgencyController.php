...
class AdminAdAgencyController extends Controller
{
	...
	public function createSend(Request $request)
	{
		$this->validate($request, [
			...
			'login_id'		=> 'bail|required|surrogate_pair_check|emoji_check|use_char_check|max:'.config('const.agency_login_id_max_length').'|unique:'.Session::get('operation_select_db').'.agencies,login_id',
	...
	public function store(Request $request)
	{
		...
		$this->validate($request, [
			'agency'		=> 'bail|required|surrogate_pair_check|emoji_check',
			'login_id'		=> 'bail|required|surrogate_pair_check|emoji_check|use_char_check|max:'.config('const.agency_login_id_max_length').'|unique:'.Session::get('operation_select_db').'.agencies,login_id,'.$edit_id.',id',
			'password'		=> 'bail|required|surrogate_pair_check|emoji_check|use_char_check|max:'.config('const.password_max_length').'|min:'.config('const.password_length'),
			'description'	=> 'bail|surrogate_pair_check|emoji_check',
		]);
