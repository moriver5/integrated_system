...
class AdminMelmagaAnalyticsController extends Controller
{
	...
	public function index()
	{
		...
		//メルマガ全配信データ取得
		$access_all_data = DB::connection(Session::get('operation_select_db'))->table("melmaga_history_logs")->select("melmaga_id", DB::raw("count(melmaga_id) as count"), "sort_date")->groupBy("melmaga_id", "sort_date")->orderBy("sort_date", "desc")->paginate(config('const.admin_client_list_limit'));
		...
		//メルマガ配信閲覧済データ取得
		$access_data = DB::connection(Session::get('operation_select_db'))->select("select melmaga_id,count(melmaga_id) as count from melmaga_history_logs where read_flg = 1 group by melmaga_id,sort_date order by sort_date desc limit ".config('const.admin_client_list_limit'));
		...
		//メルマガ配信未閲覧データ取得
		$access_data = DB::connection(Session::get('operation_select_db'))->select("select melmaga_id,count(melmaga_id) as count from melmaga_history_logs where read_flg = 0 group by melmaga_id,sort_date order by sort_date desc limit ".config('const.admin_client_list_limit'));
		foreach($access_data as $lines){
			if( !empty($listMelmaga[$lines->melmaga_id]) ){
	...
	public function viewVisited($melmaga_id)
	{
		...
		//メルマガ閲覧済データ取得
		$access_data = DB::connection(Session::get('operation_select_db'))->table("melmaga_history_logs")->select("client_id")->where("read_flg", 1)->where('melmaga_id', $melmaga_id)->orderBy("sort_date")->paginate(config('const.admin_client_list_limit'));
	...
	public function viewUnseen($melmaga_id)
	{
		...
		//メルマガ未閲覧データ取得
		$access_data = DB::connection(Session::get('operation_select_db'))->table("melmaga_history_logs")->select("client_id")->where("read_flg", 0)->where('melmaga_id', $melmaga_id)->orderBy("sort_date")->paginate(config('const.admin_client_list_limit'));
