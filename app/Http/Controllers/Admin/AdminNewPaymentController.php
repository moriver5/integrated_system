...
class AdminNewPaymentController extends Controller
{
	...
	public function index($year = null)
	{
		...
		for($month=1;$month<=12;$month++){
			...
			foreach($db_data as $lines){
				...
				$first_pay_month = sprintf("%d", preg_replace("/\d{4}(\d{2})\d{2}/", "$1", $lines->first_pay_month));
				//登録月に初回データがあるとき
				if( $first_pay_month == $month ){
					//初回入金者
	...
	public function monthAnalysis($year, $month)
	{
		...
		//入金構成データ取得
		$db_data = DB::connection(Session::get('operation_select_db'))->select("select substring(users.regist_date,1,8) as user_regist_date, payment_logs.login_id, sort_date as first_pay_date "
						. "from payment_logs inner join users on payment_logs.login_id = users.login_id "
						. "where "
						. "payment_logs.pay_count = 1 and "
						. "payment_logs.status in('0','3') and "
		...
		for($day=1;$day<=$last_day;$day++){
			...
			foreach($db_data as $lines){
				...
				$first_pay_day = sprintf("%d", preg_replace("/\d{4}\d{2}(\d{2})/", "$1", $lines->first_pay_date));

				//登録月に初回データがあるとき
				if( $first_pay_day == $day ){
					//初回入金者
					...
					//経過日数の計算
					$regist_date = strtotime(preg_replace("/(\d{4})(\d{2})(\d{2})/", "$1-$2-$3", $lines->user_regist_date));
					$first_pay_day = strtotime(preg_replace("/(\d{4})(\d{2})(\d{2})/", "$1-$2-$3", $lines->first_pay_date));
 ...
 */
		...
		$listData = $query->join('users', 'users.login_id', '=', 'payment_logs.login_id')
				->select('users.pay_datetime as pay_date', 'users.regist_date as user_regist_date','users.mail_address as email','users.id as client_id','payment_logs.*')
				->whereIn('payment_logs.status', ['0', '3'])
				->where('payment_logs.pay_count', 1)
				->where(DB::connection(Session::get('operation_select_db'))->raw('substr(users.sort_pay_datetime,1,8)'), $year.sprintf("%02d", $month).sprintf("%02d", $day))
