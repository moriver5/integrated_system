...
class AdminImgUploadController extends Controller
{
	...
	public function uploadImg(Request $request)
	{
		...
		DB::connection(Session::get('operation_select_db'))->transaction(function() use($file){
			...
			$imgpath = config('const.public_full_path').config('const.list_domain_const')[Session::get('operation_select_db')].'/'.config('const.upload_img_path');

			//画像の保存先を移動
			//保存先：/data/www/jray/public/upload_images
			$file->move($imgpath.'/', $img_name);

			//アップロード時の画像名を変更
			rename($imgpath.'/'.$img_name, $imgpath.'/'.$id.'.'.$extension);
		});
	...
	public function deleteImg(Request $request)
	{
		...
		foreach($listDelImg as $img){
			//ディレクトリから画像削除
			system("rm ".config('const.public_full_path').config('const.list_domain_const')[Session::get('operation_select_db')].'/'.config('const.upload_img_path').'/'.$img);
