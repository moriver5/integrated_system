...
class KeibaController extends Controller
{
	protected $log_obj;
...
//print $app['config']['view.paths'][0].'/'.$app['config']['const']['list_site_const'][$_SERVER['SERVER_NAME']]."<br>";
			//ファイル名までのフルパス名生成
			$file_path		 = config('const.public_full_path').'/'.$_SERVER['SERVER_NAME'].'/'.config('const.landing_dir_path').'/'.$id.'/sp/'.$file;

			//Storage::disk('local')->get($file_path))でのランディングページまでのパス
			$storage_path	 = config("const.public_dir_path")."/".$_SERVER['SERVER_NAME'].'/'.config("const.landing_dir_path")."/{$id}/sp/{$file}";
//echo $file_path."<br>";
...
//			$this->pv_log_obj->addPvLogDb(config('const.display_list')['pc_top']);
			...
			//ファイル名までのフルパス名生成
			$file_path		 = config('const.public_full_path').'/'.$_SERVER['SERVER_NAME'].'/'.config('const.landing_dir_path').'/'.$id.'/pc/'.$file;

			//Storage::disk('local')->get($file_path))でのランディングページまでのパス
			$storage_path	 = config("const.public_dir_path")."/".$_SERVER['SERVER_NAME'].'/'.config("const.landing_dir_path")."/{$id}/pc/{$file}";
		}
		...
		//ファイルが存在しないなら
			...
			//的中データを取得
			[$db_data, $list_hit_data] = Utility::getHitAchievements();
	...
	public function forget()
	{
		...
		if( $request->input("status") == config("const.forget_end_status") ){
			$disp_data['send_msg']	= __('messages.password_forget');
		}
