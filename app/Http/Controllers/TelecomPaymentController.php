...
class TelecomPaymentController extends Controller
{
	 ...
	 */
	public function telecomCreditPaymentResponse(Request $request)
	{
		$listResonse = [];
...
//		$this->credit_log_obj->addLog(implode(",", $listResonse));
		try {
			//vendor/laravel/framework/src/Illuminate/Database/DatabaseManager.phpのconnection内でアクセス元ドメインでDB自動切換え
...
//error_log("nodata\n",3,"/data/www/storage/siteo/storage/logs/nishi_log.txt");
			...
			try{
				//決済完了
//				if( $listResonse['rel'] == 'yes' && $listUserInfo['amount'] == $listResonse['money'] ){
				if( $listResonse['rel'] == 'yes' ){
...
//					$stmt->bindValue(":pay_amount", (int)$listResonse['money'], PDO::PARAM_INT);
					$stmt->bindValue(":pay_amount", (int)$listUserInfo['amount'], PDO::PARAM_INT);
...
//error_log($user_db_data->login_id.":".$user_db_data->pay_count.":error\n",3,"/data/www/jray/storage/logs/nishi_log.txt");
	...
	public function telecomNetbankPaymentResponse(Request $request)
	{
		//現在の時刻を取得
		$now_date = Carbon::now();
...
//error_log("pdo error\n",3,"/data/www/jray/storage/logs/nishi_log.txt");
		...
		try{ 
			...
			$stmt->bindValue(":mail_address", $listResonse['email']);
			$stmt->bindValue(":sendid", $listResonse['sendid']);
			$stmt->bindValue(":order_id", $listResonse['option']);
			$stmt->bindValue(":status", config('const.settlement_result')[3]);
			$stmt->execute();
//			throw new \Exception("テスト例外エラー");
...
//				return '0005';
			...
			try{
				...
				//振込結果：入金済
				if( $listResonse['status'] == 'TEST' && 
					$listResonse['banktransfer'] == 'yes' && 
					$listResonse['rel'] == 'yes' && 
					$listUserInfo['amount'] == $listResonse['money'] ){

					//pay_type: 1→銀行振込　2→クレジット 3→ネットバンク　0→管理手動(管理画面)
					//payment_logsテーブルに決済情報を登録
					$this->_updateNetbankProductInfo($dbh, $listUserInfo, $listResonse, $now_date, $listUserInfo['pay_count'], config('const.settlement_result')[3]);
//					throw new \Exception("テスト例外エラー");
...
//					throw new \Exception("テスト例外エラー");
				...
				//振込結果：エラー
					...
					//payment_logsテーブルに決済情報を登録
					$this->_updateNetbankProductInfo($dbh, $listUserInfo, $listResonse, $now_date, $listUserInfo['pay_count'], config('const.settlement_result')[6]);
...
//error_log($user_db_data->login_id.":".$user_db_data->pay_count.":error\n",3,"/data/www/jray/storage/logs/nishi_log.txt");
			//payment_logsテーブルに決済情報を登録
			$this->_updateNetbankProductInfo($dbh, $user_db_data, $listResonse, $now_date, $user_db_data->pay_count, config('const.settlement_result')[6]);
...
//			return '0004';
	...
	private function _updateProductInfo($dbh, $user, $listResonse, $now_date, $pay_count, $status){
		//payment_logsテーブルを更新
		$stmt = $dbh->prepare("update payment_logs set pay_count = :pay_count, status = :status, regist_date = :regist_date, tel = :tel, email = :email, username = :username, cont = :cont where order_id = :order_id");
		$stmt->bindValue(":pay_count", $pay_count);
		$stmt->bindValue(":status", $status);
		$stmt->bindValue(":regist_date", $now_date);
		$stmt->bindValue(":order_id", $listResonse['sendpoint']);
		$stmt->bindValue(":tel", $listResonse['telno']);
		$stmt->bindValue(":email", $listResonse['email']);
		$stmt->bindValue(":username", $listResonse['username']);
		$stmt->bindValue(":cont", $listResonse['cont']);
		$stmt->execute();
	...
	private function _updateNetbankProductInfo($dbh, $user, $listResonse, $now_date, $pay_count, $status){
		//payment_logsテーブルを更新
		$stmt = $dbh->prepare("update payment_logs set pay_count = :pay_count, status = :status, regist_date = :regist_date, email = :email where order_id = :order_id");
		$stmt->bindValue(":pay_count", $pay_count);
		$stmt->bindValue(":status", $status);
		$stmt->bindValue(":regist_date", $now_date);
		$stmt->bindValue(":order_id", $listResonse['option']);
